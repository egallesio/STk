    ...
    char **argv;		/* Argument strings. */
{
	...
	if (cmdLength == 0 || strcmp(args[1], "()") == 0) {
	    ...
	    STk_add_callback(args[0], selName, targetName, STk_get_NIL_value());
#else
	if (cmdLength == 0) {
	    Tk_DeleteSelHandler(tkwin, selection, target);
#endif
	  ...
	  if (!STk_valid_callback(args[1], &p) || (p == NULL)) {
...
#ifdef STk_CODE
	    STk_add_callback(args[0], selName, targetName, p);
#endif
	}
	return TCL_OK;
...
#ifdef STk_CODE
		STk_sharp_dot_result(interp, Tk_PathName(infoPtr->owner));
#else
...
#ifdef STk_CODE
	  if (!STk_valid_callback(script, &p) || (p == NULL)) {
	    Tcl_AppendResult(interp, "bad closure specification \"",
			     script, "\"", (char *) NULL);
	    return TCL_ERROR;
	  }
...
#ifdef STk_CODE
	STk_add_callback(args[0], selName, "", p);
#endif
	return TCL_OK;
    } else {
	sprintf(interp->result,
