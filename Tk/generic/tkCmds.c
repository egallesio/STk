...
#ifdef STk_CODE
	STk_stringify_result(interp, winPtr->nameUid);
#else
...
#define SETUP(name) \
...
#ifdef STk_CODE
	STk_stringify_result(interp, name);
#else
	interp->result = name;
#endif
	...
	SETUP("class");
#ifdef STk_CODE
	STk_stringify_result(interp, Tk_Class(window));
#else
	...
	if (window != NULL) {
#ifdef STk_CODE
	    STk_sharp_dot_result(interp, Tk_PathName(window));
	}
	else {
	  interp->result = "#f";
#else
...
#ifdef STk_CODE
	sprintf(interp->result, "#x%x", (unsigned int) Tk_WindowId(window));
#else
	...
	SETUP("ismapped");
#ifdef STk_CODE
	interp->result = Tk_IsMapped(window) ? "#t" : "#f";
#else
...
#ifdef STk_CODE
	    STk_stringify_result(interp, winPtr->geomMgrPtr->name);
	}
	else
	  interp->result = "#f";
#else
	...
	SETUP("name");
#ifdef STk_CODE
	STk_stringify_result(interp, Tk_Name(window));
#else
...
#ifdef STk_CODE
	    STk_sharp_dot_result(interp, winPtr->parentPtr->pathName);
#else
...
#ifdef STk_CODE
	STk_stringify_result(interp, Tk_PathName(window));
#else
	interp->result = Tk_PathName(window);
#endif
...
#ifdef STk_CODE
	{ 
	  char *s = (char*) STk_stringify(interp->result, 0);
	...
	if (winPtr != NULL) {
#ifdef STk_CODE
	    STk_sharp_dot_result(interp, winPtr->pathName);
#else
